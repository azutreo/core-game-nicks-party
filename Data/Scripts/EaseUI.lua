...
local function verifyEaseFluidUI(fluidUIElement, property, subProperty, goal, easeDuration, easingEquation, easingDirection)
	if(not Object.IsValid(fluidUIElement)) then
		return false, "Attempting to ease an object that does not exist"
	elseif(not fluidUIElement:IsA("Script")) then
		return false, "Attempting to ease an object that is not a FluidUI Element"
	elseif((property ~= "Percent") and (property ~= "Pixels")) then
		return false, "Attempting to ease an invalid FluidUI Element property"
	elseif((subProperty ~= "x") and (subProperty ~= "y")) then
		return false, "Attempting to ease an invalid FluidUI Element subproperty"
	elseif(type(easeDuration) ~= "number") then
		return false, "Attempting to ease with an invalid amount of time"
	elseif(type(goal) ~= "number") then
		return false, "Attempting to ease to a goal that is not a number"
...
function Module.EaseFluidUI(fluidUIElement, property, subProperty, goal, easeDuration, easingEquation, easingDirection)
	if(type(easeDuration) == "nil") then easeDuration = 1 end
	if(type(easingEquation) == "nil") then easingEquation = Module.EasingEquation.LINEAR end
	if(type(easingDirection) == "nil") then easingDirection = Module.EasingDirection.INOUT end

	local success, response = verifyEaseFluidUI(fluidUIElement, property, subProperty, goal, easeDuration, easingEquation, easingDirection)
	assert(success, response)

	local easingFormula = Module._GetEasingEquationFormula(easingEquation, easingDirection)
	assert(easingFormula, "Attempting to ease with an invalid easing equation enum on the x axis; check that you spelled the enum correctly")

	local taskProperty = property .. subProperty
	clearFromTask(fluidUIElement, taskProperty)

	local startTime = time()
	local start = fluidUIElement.context[property][subProperty]

	local direction = ((start < goal) and 1) or -1

	wrapTask(taskProperty, fluidUIElement, function()
		if(not Object.IsValid(fluidUIElement)) then
			return clearFromTask(fluidUIElement, taskProperty)
		end

		local currentTime = time() - startTime

		if(currentTime >= easeDuration) then
			fluidUIElement.context[property][subProperty] = CoreMath.Round(goal)
			fluidUIElement.context.Update()

			return clearFromTask(fluidUIElement, taskProperty)
		end

		fluidUIElement.context[property][subProperty] = easingFormula(currentTime, start, direction * math.abs(goal - start), easeDuration)
		fluidUIElement.context.Update()
	end)
end

function Module.EaseFluidUIPercentX(fluidUIElement, goal, easeDuration, easingEquation, easingDirection)
	Module.EaseFluidUI(fluidUIElement, "Percent", "x", goal, easeDuration, easingEquation, easingDirection)
end

function Module.EaseFluidUIPercentY(fluidUIElement, goal, easeDuration, easingEquation, easingDirection)
	Module.EaseFluidUI(fluidUIElement, "Percent", "y", goal, easeDuration, easingEquation, easingDirection)
end

function Module.EaseFluidUIPixelsX(fluidUIElement, goal, easeDuration, easingEquation, easingDirection)
	Module.EaseFluidUI(fluidUIElement, "Pixels", "x", goal, easeDuration, easingEquation, easingDirection)
end

function Module.EaseFluidUIPixelsY(fluidUIElement, goal, easeDuration, easingEquation, easingDirection)
	Module.EaseFluidUI(fluidUIElement, "Pixels", "y", goal, easeDuration, easingEquation, easingDirection)
end
